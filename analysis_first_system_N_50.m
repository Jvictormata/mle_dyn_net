...
%%%% MLE u1 u3
pyData = py.numpy.load("results/cov/theta_opt_u1u3_100_rel_arx_init_TR_N_50.npy");  
...
%%%% MLE u3
pyData = py.numpy.load("results/cov/theta_opt_u3_100_rel_arx_init_TR_N_50.npy");  
...
nf = [2 2]; 
nb = [2 2];
nc = [2];  % 1 was the order for barH which led to the best fit values for prediction on the validation data for system 1 and N=50
nd = [2];  % 1 was the order for barH which led to the best fit values for prediction on the validation data for system 1 and N=50
nk = [1 1];
opt = bjOptions;
opt.SearchOptions.Tolerance = tolerance;
